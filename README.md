...
## Architecture: Skills + Agents (dual install)

The board runs on a **dual architecture**:

- **Skills** (`skills/csuite-*.md`) — invokable via `/csuite-cfo`, `/csuite-coach` etc. as standalone slash commands. Live in the main Claude Code context.
- **Agents** (`agents/csuite-*.md`) — invokable via the `Agent` tool with `subagent_type: csuite-cfo` etc. Run in isolated subagent contexts. This is what the **Moderator uses for true parallel, independent board rounds** — every Round-1 and Round-2 contribution is generated by a real isolated subagent call, not roleplay from a single context.

Without the agents installed, `/csuite` falls back to a workaround (general-purpose subagents with embedded role prompts) — works, but slower and less clean.

## Installation

Both layers should be installed. Skills and agent definitions are pure Markdown files — no build step.

```bash
git clone https://github.com/mhmarkhoffmann-lob/csuite-agent-builder.git
cd csuite-agent-builder

# Skills (slash-command standalone usage)
...
# Agents (subagent registry — required for true parallel board rounds)
mkdir -p ~/.claude/agents
cp agents/csuite-*.md ~/.claude/agents/
```

Restart Claude Code after install so the new agent definitions are picked up.
